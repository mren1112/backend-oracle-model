...
const DataController = {
    ...
    async exam1(req, res) {
        ...
        try {
            ...
            if (!STD_CODE) {
                return res.status(200).json({ success: false, messageEN: `เข้าถึงข้อมูลไม่สำเร็จ. `, messageEN: `Unauthorized data access. ` });
            }
            ...
            const result = await DbTx.withTransaction(async (connection) => {
                ...
                if (!counterInfo) {
                    return { success: false, messageTH: 'ไม่พบข้อมูล', messageEN: 'Data not found.' };
                }

                const sql = `  `;

                const subStrId = STD_CODE.substring(0, 2);
                const { rows } = await connection.execute(   // ✅ เปลี่ยนชื่อจาก res → rows
                    sql,
                    { p1: p1, p2: p2, params3 },
